...
int main()
{
	char buffer[100];
	time_t t1,t2;
	int client=0,listenfd, len,nreq=0;
	...
	while(1) {
		...
		if(nreq==NO_OF_REQUESTS) {
			t2= time(NULL);
			if(t2 - t1 < NO_OF_SECONDS){
				sendto(listenfd, "WAIT", MAXLINE, 0,(struct sockaddr*)&cliaddr, sizeof(cliaddr));
				sleep(NO_OF_SECONDS-(t2-t1));
			}
			t1= time(NULL);
			nreq=0;
		}
		sendto(listenfd, "RESTART", MAXLINE, 0,(struct sockaddr*)&cliaddr, sizeof(cliaddr));
	}
	printf("Exiting\n");
	close(listenfd);
}
